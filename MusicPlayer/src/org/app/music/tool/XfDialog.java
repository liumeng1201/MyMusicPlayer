...
public class XfDialog extends Dialog {
	...
	public static class Builder {
		...
		public XfDialog create() {
			...
			int count = 0;
			// 设置确定按钮
			if (setButton(layout, mPositiveButtonText, R.id.button1, dialog,
					mPositiveButtonClickListener))
				count++;
			// 设置拒绝按钮
			if (setButton(layout, mNegativeButtonText, R.id.button2, dialog,
					mNegativeButtonClickListener))
				count++;
			// 设置中间按钮
			if (setButton(layout, mNeutralButtonText, R.id.button3, dialog,
					mNeutralButtonClickListener))
				count++;
			...
			// 一个按钮时，显示两边空间
			if (count == 1) {
				layout.findViewById(R.id.leftSpacer).setVisibility(
						View.INVISIBLE);
		...
		private boolean setButton(View layout,
				CharSequence mPositiveButtonText2, int id, final Dialog dialog,
				final OnClickListener listener) {
